...
class IntrinsicPPO:
    def __init__(self, cfg):
        ...
        reward_model_file_name = f'{cfg.reward_save_dir}/reward_model_{cfg.intrinsic_module}_{cfg.env_name}.pt'
        obs_dim = self.obs_shape[0]
        action_dim = self.action_shape[0]

        self.intrinsic_module = cfg.intrinsic_module 
        self.update_reward_model = False 
        if cfg.intrinsic_module == 'gail':
            self.reward_model = GAIL(
                                            obs_dim,
                                            action_dim,
                                            device='cuda:0',
                                            lr=3e-4)
        ...
        if cfg.intrinsic_module == 'm_cond_gail':
            self.reward_model = mCondGAIL(
                                            obs_dim,
                                            action_dim,
        ...
        if cfg.intrinsic_module == 'm_cond_reg_gail':
            self.reward_model = mCondRegGAIL(
                                            obs_dim,
                                            action_dim,
                                            measure_dim=self.cfg.num_dims,
                                            reg_loss_fn=self.cfg.auxiliary_loss_fn, 
        ...
        if cfg.intrinsic_module == 'vail':
            self.reward_model = VAIL(
                                            obs_dim,
                                            action_dim,
                                            i_c=0.5,
                                            device='cuda:0',
        ...
        if cfg.intrinsic_module == 'm_cond_icm':
            self.reward_model = mCondICM(
                                        obs_dim,
                                        action_dim,
                                        measure_dim=self.cfg.num_dims,
                                        inverse_lr=3e-4,
        ...
        if cfg.intrinsic_module == 'm_reg_icm':
            self.reward_model = mRegICM(
                                        obs_dim,
                                        action_dim,
                                        measure_dim=self.cfg.num_dims,
                                        reg_loss_fn=self.cfg.auxiliary_loss_fn,
        ...
        if cfg.intrinsic_module == 'm_cond_reg_icm':
            self.reward_model = mCondRegICM(
                                        obs_dim,
                                        action_dim,
                                        measure_dim=self.cfg.num_dims,
                                        reg_loss_fn=self.cfg.auxiliary_loss_fn,
    ...
    def train(self, vec_env, num_updates, rollout_length, calculate_dqd_gradients=False, move_mean_agent=False,
        ...
        next_obs, info_ = vec_env.reset_return_info()
        next_measure = info_['measures']
        self.next_obs = torch.from_numpy(np.asarray(copy.deepcopy(next_obs))).to(self.device)
        self.next_measure = torch.from_numpy(np.asarray(copy.deepcopy(next_measure))).to(self.device)                                 
            ...
            with torch.inference_mode():
                ...
                if self.cfg.use_wandb:
                    ...
                    if self.update_reward_model:
                        wandb.log(
                            {
                                "reward_model/reward_loss": reward_loss
                            }
                        )
