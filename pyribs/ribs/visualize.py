    ...
    if archive.measure_dim == 1:
        ...
        if emitter_loc:
            if archive.measure_dim == 2:
                point = Circle(emitter_loc, 0.008, color='r', zorder=100)
                ax.add_patch(point)
        ...
        if forces:
            if archive.measure_dim == 2:
                (f1, f2) = forces
                # m1 grad coeff
                plt.quiver(0.5, 0.02, f1 * 4, 0, scale=50, width=0.01, headlength=4.5, color='r', zorder=100000)
                # m2 grad coeff
                plt.quiver(0.02, 0.5, 0.0, f2 * 4.0, scale=50, width=0.01, headlength=4.5, color='r', zorder=100000)
